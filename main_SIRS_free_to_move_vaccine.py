...
class SIRS_Movement_Simulation:
    ...
    Includes vaccination with adjustable effectiveness and gradual rollout.
    """
    ...
    def __init__(self, master):
        ...
        self.vaccination_rate = 0.5  # Target percentage of population to be vaccinated (0.0 to 1.0)
        self.vaccination_speed = 0.005  # Probability per step that an eligible agent is vaccinated
    ...
    def create_sliders(self):
        ...
        ttk.Label(self.main_frame, text="Target Vaccination Rate (%)").grid(row=10, column=1, sticky="w")
        ...
        ttk.Label(self.main_frame, text="Vaccination Speed").grid(row=12, column=1, sticky="w")
        self.vaccination_speed_slider = tk.Scale(self.main_frame, from_=0.0, to=0.05, resolution=0.001,
                                                 orient="horizontal", command=self.update_vaccination_speed, length=200)
        self.vaccination_speed_slider.set(self.vaccination_speed)
        self.vaccination_speed_slider.grid(row=13, column=1, sticky="ew")

        ttk.Label(self.main_frame, text="Vaccine Effectiveness (%)").grid(row=14, column=1, sticky="w")
        ...
        self.effectiveness_slider.grid(row=15, column=1, sticky="ew")
    ...
    def update_vaccination_speed(self, val):
        self.vaccination_speed = float(val)
    ...
    def step(self):
        ...
        # 2. Gradual vaccination
        current_vaccinated_count = sum(1 for a in self.agents if a['is_vaccinated'])
        target_vaccinated_count = self.agent_count * self.vaccination_rate
        if current_vaccinated_count < target_vaccinated_count:
            eligible_agents = [a for a in self.agents if a['status'] == 'S' and not a['is_vaccinated']]
            if eligible_agents:
                if random.random() < self.vaccination_speed:
                    agent_to_vaccinate = random.choice(eligible_agents)
                    agent_to_vaccinate['is_vaccinated'] = True

        # 3. Check for interactions and apply state changes
